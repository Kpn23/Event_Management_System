...
def add_user(password, user_type, name, phone, email, gender, age):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO user (password, user_type, name, phone_number, email_address, gender, age) VALUES (?, ?, ?, ?, ?, ?, ?)', 
...
def add_ticket(schedule_id, price):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO ticket (schedule_id, price) VALUES (?, ?)', (schedule_id, price))
    conn.commit()
    conn.close()
...
def add_ticket_sales(user_id, ticket_id, sale_time):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO ticket_sales (user_id, ticket_id, sale_time) VALUES (?, ?, ?)', 
...
def add_event(venue_id, event_name, event_description):
    ...
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO event (venue_id, event_name, event_description, created_at) VALUES (?, ?, ?, ?)', 
                   (venue_id, event_name, event_description, created_at))
    conn.commit()
    conn.close()
    print("Event added successfully.")

def add_event_schedule(event_id, schedule_name, schedule_start_time, schedule_end_time, schedule_description):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO event_schedule (event_id, schedule_name, schedule_start_time, schedule_end_time, schedule_description) VALUES (?, ?, ?, ?, ?)', 
                   (event_id, schedule_name, schedule_start_time, schedule_end_time, schedule_description))
    conn.commit()
    conn.close()
...
def add_venue(location, capacity):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO venue (location, capacity) VALUES (?, ?)', 
                   (location, capacity))
    conn.commit()
    conn.close()
...
def add_feedback(ticket_id, comment, ratings):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO feedback (ticket_id, comment, ratings) VALUES (?, ?, ?)', 
...
def view_user():
    ...
    if rows:
        for row in rows:
            print(f"user_id: {row[0]}, password: {row[1]}, user_type: {row[2]}, name: {row[3]}, phone_number: {row[4]}, email_address: {row[5]}, gender: {row[6]}, age: {row[7]}")
    else:
        print("No user found.")
...
def view_ticket_sales():
    ...
    if rows:
        for row in rows:
            print(f"sales_id: {row[0]}, user_id: {row[1]}, ticket_id: {row[2]}, sale_time: {row[3]}")
    else:
...
def view_event():
    ...
    if rows:
        for row in rows:
            print(f"event_id: {row[0]}, venue_id: {row[1]}, event_name: {row[2]}, event_description: {row[3]}, created_at: {row[4]}, location: {row[5]}, capacity:{row[6]}")
    else:
...
def view_event_schedule():
    ...
    if rows:
        for row in rows:
            print(f"schedule_id: {row[0]}, event_id: {row[1]}, schedule_name: {row[2]}, schedule_start_time: {row[3]}, schedule_end_time: {row[4]}, schedule_description: {row[5]}")
    else:
...
def view_feedback():
    ...
    if rows:
        for row in rows:
            print(f"event_name: {row[4]},feedback_id: {row[0]}, ticket_id: {row[1]}, comment: {row[2]}, ratings: {row[3]}")
    else:
        print("No feedback found.")
############################################################################^^^^VIEW FUNCTION^^^^^^^^^################################################################################################################################################
def update_user(user_id, new_password, new_user_type, new_name, new_phone, new_email, new_gender, new_age):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('''
        UPDATE user 
        ...
        WHERE user_id = ?;
    ''', (new_password, new_user_type, new_name, new_phone, new_email, new_gender, new_age, user_id))
    conn.commit()
    conn.close()
...
def update_event_schedule(schedule_id, new_event_id, new_schedule_name, new_start_time, new_end_time, new_description):
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('''
        ...
        WHERE schedule_id = ?;
    ''', (new_event_id, new_schedule_name, new_start_time, new_end_time, new_description, schedule_id))
    conn.commit()
    conn.close()
...
def search_records():
    ...
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM user WHERE name LIKE ? OR email_address LIKE ?", ('%' + criteria + '%', '%' + criteria + '%'))
...
def generate_reports():
    ...
    cursor.execute("SELECT e.event_name, COUNT(ts.ticket_id) AS total_sales FROM ticket_sales ts JOIN ticket t ON ts.ticket_id = t.ticket_id JOIN event_schedule es ON t.schedule_id = es.schedule_id JOIN event e ON es.event_id = e.event_id GROUP BY e.event_name")
...
def analyze_data():
    ...
    cursor.execute("SELECT user_id, COUNT(ticket_id) AS tickets_purchased FROM ticket_sales GROUP BY user_id")
...
def export_data_to_csv():
    ...
    with open('/Users/superdayuanjingzhi/Downloads/ticket_sales_report.csv', 'w', newline='') as file:
...
def edit_PA(name, password,new_user_name, new_user_password,new_user_pn,new_user_email,new_user_gender,new_user_age):
    ...
    cursor.execute('''UPDATE user
                   ...
                   , (new_user_name, new_user_password,new_user_pn,new_user_email,new_user_gender,new_user_age,name,password))
    conn.commit()
    conn.close()
    print("User information updated successfully.")
...
def user_view_event():
    conn = sqlite3.connect('project_database.db')
    cursor = conn.cursor()
    cursor.execute('''SELECT e.event_id,e.event_name, e.event_description, es.schedule_id,es.schedule_name,
    ...
    if rows:
        for row in rows:
            print(f'event_id: {row[0]}, event_name: {row[1]}, event_description: {row[2]},\nschedule_id: {row[3]}, '
...
def check_tickets(login_user_id):
    ...
    for idx, (schedule_id, schedule_name, ticket_id, price, remaining_capacity) in enumerate(schedules):
        print(f"{idx + 1}. Schedule: {schedule_name}, Price: {price}, Remaining: {remaining_capacity}")
    ...
    if selected_schedule[4] == 0:  # Adjusted index for remaining_capacity
        print("Selected schedule has no remaining capacity. Please choose another schedule.")
        return
    
    # Reserve Tickets using schedule_id and ticket_id
    reserve_tickets(selected_schedule[0], selected_schedule[2], login_user_id)  # Use schedule_id and ticket_id here
...
def reserve_tickets(schedule_id, ticket_id, login_user_id):
    ...
    if already_reserved > 0:
        print("You have already reserved this ticket. You can only reserve one ticket of the same type at a time.")
        conn.close()
        return
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?)
    ''', (login_user_id, ticket_id, sale_time))  # Use ticket_id in the query
...
def cancel_tickets(login_user_id):
    ...
    cursor.execute(''' SELECT COUNT(*) FROM ticket_sales WHERE ticket_id = ? AND user_id = ? ''', (ticket_id, login_user_id))
    if cursor.fetchone()[0] == 0:
        print("Ticket not found or does not belong to you.")
        return

    # Find the schedule_id associated with the ticket
    cursor.execute(''' SELECT schedule_id FROM ticket WHERE ticket_id = ? ''', (ticket_id,))
    schedule_id = cursor.fetchone()[0]

    # Delete the ticket sale
    cursor.execute('DELETE FROM ticket_sales WHERE ticket_id = ? AND user_id = ?', (ticket_id, login_user_id))
...
def main():
    ...
    if result is not None and result[0] == '123':
        ...
        conn = sqlite3.connect('project_database.db')
        cursor = conn.cursor()
        cursor.execute('''INSERT INTO user (password, user_type, name, phone_number, email_address, gender, age)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        if table_choice == 1:  # User Management
            ...
            if function_choice == 1:  # View user
                ...
                if cursor.fetchone()[0] == 0:
                    ...
                    new_user_type = int(input("Enter new user type (0 for Admin, 1 for User): "))
                    update_user(user_id, new_password, new_user_type, new_name, new_phone, new_email, new_gender, new_age)
            ...
            if function_choice == 1:  # View event
                ...
                schedule_start_time = input("Enter schedule start time (YYYY-MM-DD HH:MM:SS): ")
                schedule_end_time = input("Enter schedule end time (YYYY-MM-DD HH:MM:SS): ")
                schedule_description = input("Enter schedule description: ")
                add_event_schedule(event_id, schedule_name, schedule_start_time, schedule_end_time, schedule_description)
            elif function_choice == 3:  # Edit event
                event_id = input("Enter event_id to be updated: ")
                # Check if event exists
                conn = sqlite3.connect('project_database.db')
                cursor = conn.cursor()
                cursor.execute('SELECT COUNT(*) FROM event WHERE event_id = ?', (event_id,))
                if cursor.fetchone()[0] == 0:
                    ...
                    update_event(event_id, new_venue_id, new_event_name, new_event_description)
                conn.close()
                ...
                if cursor.fetchone()[0] == 0:
                    ...
                    conn = sqlite3.connect('project_database.db')
                    cursor = conn.cursor()
                    cursor.execute('SELECT COUNT(*) FROM event_schedule WHERE schedule_id = ?', (schedule_id,))
                    if cursor.fetchone()[0] == 0:
                            print("Schedule not found")
                    else:
                            new_schedule_name = input("Enter new schedule_name: ")
                            new_start_time = input("Enter new start_time(YYYY-MM-DD HH:MM:SS): ")
                            new_end_time = input("Enter new end_time(YYYY-MM-DD HH:MM:SS): ")
                            new_description = input("Enter new new_description: ")
                            update_event_schedule(schedule_id, event_id, new_schedule_name, new_start_time, new_end_time, new_description)
                    conn.close()
            ...
            if function_choice == 1:  # View venue
                ...
                conn = sqlite3.connect('project_database.db')
                cursor = conn.cursor()
                cursor.execute('SELECT COUNT(*) FROM venue WHERE venue_id = ?', (venue_id,))
                if cursor.fetchone()[0] == 0:
            ...
            if function_choice == 1:  # View ticket
                ...
                conn = sqlite3.connect('project_database.db')
                cursor = conn.cursor()
                cursor.execute('SELECT COUNT(*) FROM ticket WHERE ticket_id = ?', (ticket_id,))
                if cursor.fetchone()[0] == 0:
            ...
            if function_choice == 1:  # View feedback
                ...
                conn = sqlite3.connect('project_database.db')
                cursor = conn.cursor()
                cursor.execute('SELECT COUNT(*) FROM feedback WHERE feedback_id = ?', (feedback_id,))
                if cursor.fetchone()[0] == 0:
                    ...
                    update_feedback(feedback_id, new_ticket_id, new_comment, new_ratings)
                conn.close()
            ...
            if option == 1:
                ...
                if not sales_records:
                    ...
                    for idx, record in enumerate(sales_records):
                        ...
                        print(f"{idx + 1}. Sales ID: {sales_id}, Event Name: {event_name}")
